...
import { bullQueue } from "../worker/queue";
  ...
  try {
    ...
    if (idempotencyKey) {
      const existing = await pool.query(
        "SELECT id FROM jobs WHERE queue_id = $1 AND idempotency_key = $2",
    ...
    const result = await pool.query(
      `INSERT INTO jobs (id, queue_id, type, payload, priority, status, available_at, attempts, max_attempts, idempotency_key)
       VALUES ($1, $2, $3, $4, $5, 'QUEUED', $6, 0, $7, $8)
       RETURNING *`,
      [id, queueId, type, payload ?? {}, priority, availableAt, maxAttempts, idempotencyKey ?? null]
    );

    const created = result.rows[0];

    await bullQueue.add("process-job", { jobId: created.id }, { removeOnComplete: true, removeOnFail: true });
